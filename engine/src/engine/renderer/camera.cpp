...
engine::perspective_camera::perspective_camera( 
    ...
    m_far_plane(far_z) 
{ 
    m_position = glm::vec3(0.0f, 0.7f, -10.0f);  
...
void engine::perspective_camera::on_update(const timestep& timestep)
{
    ...
    if(input::key_pressed(engine::key_codes::KEY_LEFT)) // left
        move(e_direction::left, timestep);
    else if(input::key_pressed(engine::key_codes::KEY_RIGHT)) // right
        move(e_direction::right, timestep);

    if(input::key_pressed(engine::key_codes::KEY_DOWN)) // down
...
{ 
    ...
    //m_position.y = glm::clamp(m_position.y, 0.7f, 0.7f);
    //m_position.x = glm::clamp(m_position.x, -1.6f, 1.6f);
