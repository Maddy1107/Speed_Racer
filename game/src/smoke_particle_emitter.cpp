...
smoke_particle_emitter::smoke_particle_emitter()
{
	s_active = false;
}
...
void smoke_particle_emitter::load_texture(char *filename)
{
	// Load the texture
...
void smoke_particle_emitter::initialise(glm::vec3 smoke_trail_start, glm::vec3 smoke_trail_direction, int num_total_particles, float width, float height)
{
...
void smoke_particle_emitter::clear()
{
	m_particles.clear();
	s_active = false;
	accumulatedTime = 0.f;
...
void smoke_particle_emitter::on_render(const engine::perspective_camera& camera, engine::ref<engine::shader> shader)
{
	if (!s_active)
		return;
...
void smoke_particle_emitter::on_update(const engine::timestep& timestep)
{
	m_num_alive = 0;
	for (unsigned int i = 0; i < m_particles.size(); i++) {
...
void smoke_particle_emitter::add_particle()
{
	particle particle;
