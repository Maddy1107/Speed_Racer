...
void grenade::initialise(engine::ref<engine::game_object> object)
{
	m_object = object;
...
void grenade::shoot(engine::ref<engine::game_object> object, float kick)
{
	...
	m_object->set_position(object->position() + glm::vec3(0, 0.5, 0));
	//m_object->set_velocity(25.0f * camera.front_vector());
	glm::vec3 force = object->forward() * kick;
	...
	m_theta = engine::PI / 2.f - acos(object->forward().y);
	m_phi = atan2(object->forward().x, object->forward().z);
}

void grenade::on_update(const engine::timestep& time_step)
{
...
void grenade::on_render(const engine::ref<engine::shader>& shader)
{
